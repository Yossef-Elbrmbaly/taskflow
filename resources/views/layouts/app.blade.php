...
{{-- لو المستخدم مسجل دخول وعلى صفحة todos → sidebar layout --}}
@if (auth()->check() && request()->routeIs('todos'))

    <body class="bg-[#0d0d0f] text-white antialiased flex min-h-screen">

        {{-- ── SIDEBAR (desktop only) ── --}}
        <aside
            class="hidden lg:flex w-[240px] min-h-screen bg-[#141417] border-r border-white/[0.06] flex-col px-4 py-6 fixed left-0 top-0 bottom-0 z-50">
            ...
            <div class="mt-auto border-t border-white/[0.06] pt-4">
                <div class="flex items-center gap-3 px-2 py-2 rounded-xl">
                    <div
                        class="w-8 h-8 rounded-full bg-gradient-to-br from-[#7c6aff] to-[#ff6a6a] flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
        ...
        {{-- ── MOBILE TOP BAR ── --}}
        <div
            class="lg:hidden fixed top-0 left-0 right-0 z-50 bg-[#141417] border-b border-white/[0.06] px-4 h-14 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <div
                    class="w-7 h-7 rounded-full bg-gradient-to-br from-[#7c6aff] to-[#ff6a6a] flex items-center justify-center text-[10px] font-bold text-white">
                    {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                </div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
        ...
        {{-- Main content --}}
        {{-- desktop: shifted right by sidebar | mobile: shifted down by top bar --}}
...
@else
    {{-- ── الـ layout العادي لباقي الصفحات ── --}}

    <body class="bg-[#fafafa] text-gray-900 antialiased">

        <nav class="w-full top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
            <div class="max-w-7xl mx-auto px-6 lg:px-8 py-3.5 flex items-center justify-between">
                ...
                <a wire:navigate href="/" class="flex items-center gap-2 -m-1.5 p-1.5">
                    ...
                    <span class="font-bold text-[24px] text-gray-900">taskflow</span>
                </a>
